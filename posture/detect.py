...
# 在图片列表上运行批量推理.你要预测
results_front = model(['11.jpg'], save=True)  # 返回 Results 对象列表，正面图片
results_side = model(['3.jpg'], save=True)  # 返回 Results 对象列表，反面图片
...
def check_conditions():
    ...
    flow = any(value == True for key, value in results.items())
    new_results = {'flow': flow}
